...
class UserSchema(marshmallow.Schema):
    ...
    first_name = marshmallow.fields.String(
        required=True, error_messages={"required": "first name required"})
    ...
    def validate_username(self, value):
        ...
        if re.fullmatch(r"(?![.])(?!.*[_.]{2})[a-zA-Z0-9._]+(?<![_.])",
                        value) is None:
    ...
    def validate_first_name(self, value):
        if len(value) >= 256:
            raise marshmallow.ValidationError(
                "first name must be shorter than 256 characters")

    @marshmallow.validates("last_name")
    def validate_last_name(self, value):
        if len(value) >= 256:
            raise marshmallow.ValidationError(
                "last name must be shorter than 256 characters")
