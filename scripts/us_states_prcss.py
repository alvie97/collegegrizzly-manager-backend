...
wb = load_workbook("../../imp_misc_files/all-geocodes-v2016.xlsx")
ws = wb.active
    ...
    if not state == "00":
        ...
        if not county == "000":
            ...
            if not county_subdiv == "000000":
                name = row[NAME].value
                codes[state]["counties"][county] \
                    ["subdivisions"][county_subdiv] = name
                continue

        place = row[PLACE_CODE].value
        if not place == "00000":
            name = row[NAME].value
            codes[state]["places"][place] = name
            continue

        consolidated_city = row[CONSOLIDATED_CITY].value
        if not consolidated_city == "00000":
            name = row[NAME].value
            codes[state]["consolidated_cities"][consolidated_city] = name
