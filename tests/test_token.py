...
def test_refresh_token(app, client):
  with app.app_context():
    ...
    refresh_token = RefreshToken(
        token=str(uuid4()), user_id=user.username, mapped_token=access_token)
    db.session.add(refresh_token)
    db.session.commit()
    refresh_token = refresh_token.token

  client.set_cookie("localhost", "access_token", access_token, httponly=True)
  client.set_cookie("localhost", "refresh_token", refresh_token, httponly=True)

  csrf_token = generate_csrf_token()
  client.set_cookie("localhost", "x-csrf-token", csrf_token, httponly=True)

  response = client.post(
      "/api/token/refresh_access_token", headers={"x-csrf-token": csrf_token})
