...
def test_scholarship_programs(client, scholarship_id):
  ...
  response = client.delete(
      url, json={"programs": [programs["programs"][1]["name"]]})
  response_json = response.get_json()
...
def test_scholarships_needed(client, scholarship_id, college_id):
  url = "scholarships/" + scholarship_id + "/api/scholarships_needed"
  create_scholarship_url = "colleges/" + college_id + "/api/scholarships"
  response = client.post(
  ...
  response = client.post(
      create_scholarship_url, json={"name": "scholarship test _2"})
  response_json = response.get_json()
  ...
  response = client.post(
      "/api/scholarships/" + scholarship_id + "/api/scholarships_needed",
  ...
  assert response_json["scholarships_needed"][0][
      "public_id"] == scholarship_test_1
  assert response_json["scholarships_needed"][1][
      "public_id"] == scholarship_test_2

  response = client.delete(
      url, json={"scholarships_needed": [scholarship_test_2]})
  response_json = response.get_json()
